...
Generated by the bundled [architecture-diagram](catalog/skills/architecture-diagram/) skill — self-contained HTML+SVG with PNG/PDF export buttons:

- [`docs/architecture.html`](docs/architecture.html) — full system architecture (Browser → Routers → Services → Storage → Targets)
- [`docs/apply-flow.html`](docs/apply-flow.html) — what happens when you click Apply (cart → snapshot → write → log → response)

Inline mermaid versions below.
